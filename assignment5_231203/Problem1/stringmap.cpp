 ...
 */
void StringMap::rehash(int n) {
   // TODO
   count = 0;
   int nBucketsNew = nBuckets;
   StringMap::KeyValuePair *bucketsNew = buckets;
   buckets = new KeyValuePair[n];
   nBuckets = n;
   for(int i = 0; i < nBucketsNew; i++){
      if(bucketsNew[i].occupied){
        int m = insertKey(bucketsNew[i].key);
        buckets[m].value = bucketsNew[i].value;
      }
...
int StringMap::findKey(const string & key) const {
   ...
   int m = nBuckets;
   code = code % nBuckets;
   for(int i = code; i < nBuckets; i++){
      if(buckets[i].key == key && buckets[i].occupied){
         code = i;
         found = true;
         break;
      }
...
int StringMap::insertKey(const string & key) {
   ...
   if (buckets[code].occupied){
      ...
      buckets[code].occupied = true;
      saved = true;
   }
   if(!saved){
      cout << "ERROR: Insufficient space in hash table" << endl;
      return -1;
   }
   count++;
   return code;
